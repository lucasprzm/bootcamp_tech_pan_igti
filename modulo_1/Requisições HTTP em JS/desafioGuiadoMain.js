...
async function onSubmit(evt) {
  ...
  if (!employeeData.name || !employeeData.salary || !employeeData.role_id) {
    ...
    if (selectedItem) {
      ...
      employees.push(createItem); // adiciona o elemento no array
      renderData(); // carrega a lista novamente
      selectItem(createItem, listEl.lastChild); // seleciona o item e marca
      listEl.lastChild.scrollIntoView(); // leva a navegação até o item
    }
  }
}
